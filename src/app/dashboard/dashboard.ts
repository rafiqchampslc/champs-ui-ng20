...
export type MigrationRatesPerThousandChartOptions = {
  series: ApexAxisChartSeries;
  chart: ApexChart;
  xaxis: ApexXAxis;
  yaxis: ApexYAxis;
...
export type PregnancyOutcomeStackedChartOptions = {
  series: ApexAxisChartSeries;
  chart: ApexChart;
  plotOptions: ApexPlotOptions;
  xaxis: ApexXAxis;
...
export type PregnancyOutcomePercentChartOptions = {
  series: ApexAxisChartSeries;
  chart: ApexChart;
  plotOptions: ApexPlotOptions;
  xaxis: ApexXAxis;
...
export type CumulativeUnder5DeathsChartOptions = {
  series: ApexAxisChartSeries;
  chart: ApexChart;
  xaxis: ApexXAxis;
  yaxis: ApexYAxis;
...
cumulativeUnder5DeathsChartOptions: CumulativeUnder5DeathsChartOptions = {
  ...
  loadAllCharts() {
    ...
    this.loadMigrationRatesPerThousandTrend();
    ...
    this.reportService.getMaritalStatusChangeTrend(this.selectedSiteId)
      .subscribe(rows => {
        ...
        const totals = rows.map(r => {
          ...
          const c = Number(r.separations) || 0;
          const d = Number(r.widowsWidowers) || 0;
          const e = Number(r.reunions) || 0;
          return a + b + c + d + e;
        });
  ...
  loadBirthsByMotherAgeTrend() {
    ...
    this.reportService.getBirthsByMotherAgeTrend(this.selectedSiteId)
      .subscribe(rows => {
        ...
        const d35_39 = rows.map(r => r.births35_39 + r.births40_44 + r.births45_49);
        //const d40_44 = rows.map(r => r.births40_44);
        //const d45_49 = rows.map(r => r.births45_49);
